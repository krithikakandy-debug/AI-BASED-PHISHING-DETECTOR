...
function analyzeUrl(rawUrl) {
  ...
  if (!isHttps) {
    score += 20;
    riskFactors.push("No HTTPS — connection is not encrypted");
  }

  const isIp = /^\d{1,3}(\.\d{1,3}){3}$/.test(hostname);
  details["IP Address"] = isIp ? "Yes — suspicious" : "No — uses domain";
  if (isIp) {
    score += 30;
    riskFactors.push("Uses raw IP address instead of domain name");
  }
  ...
  if (subCount > 2) {
    score += (subCount - 2) * 15;
    riskFactors.push(`Excessive subdomains (${subCount} levels)`);
  }
  ...
  if (matched.length > 0) {
    score += Math.min(40, matched.length * 10);
    riskFactors.push(`Phishing keywords detected: ${matched.join(", ")}`);
  }
  ...
  if (urlLen > 100) {
    score += 20;
    riskFactors.push(`Very long URL (${urlLen} chars) — possible obfuscation`);
  } else if (urlLen > 75) {
    score += 10;
    riskFactors.push(`Long URL (${urlLen} chars)`);
  }
  ...
  if (isShortener) {
    score += 15;
    riskFactors.push("URL shortener detected — real destination is hidden");
  }

  const hasAt = rawUrl.includes("@");
  details["@ Symbol"] = hasAt ? "Found — redirect trick" : "Not found — clean";
  if (hasAt) {
    score += 20;
    riskFactors.push("@ symbol found — browser redirect trick");
  }

  const hyphens = (hostname.match(/-/g) || []).length;
  details["Hyphens in Domain"] = hyphens === 0 ? "None — clean" : `${hyphens} — suspicious`;
  if (hyphens > 0) {
    score += Math.min(15, hyphens * 5);
    riskFactors.push(`${hyphens} hyphen(s) in domain — possible brand impersonation`);
  }
  ...
  if (hasNums && !isIp) {
    score += 10;
    riskFactors.push("Numbers in domain — possible homograph attack");
  }
